...
// Pastikan direktori logos ada untuk upload logo toko
const logosDir = path.join(publicPath, 'logos');
require('fs').mkdir(logosDir, { recursive: true }, (err) => {
  if (err && err.code !== 'EEXIST') console.error(err);
});

// Pastikan direktori products ada untuk upload gambar produk
const productsDir = path.join(publicPath, 'products');
require('fs').mkdir(productsDir, { recursive: true }, (err) => {
  if (err && err.code !== 'EEXIST') console.error(err);
});
...
app.post('/api/activation/activate', async (req, res) => {
  try {
    ...
    // Handle HARDWARE_ALREADY_USED explicitly  
    if (error.code === 'HARDWARE_ALREADY_USED') {
      return res.status(409).json({
        success: false,
        error: 'HARDWARE_ALREADY_USED',
        message: error.message || 'Hardware sudah terikat dengan Serial Number lain.'
      });
    }
    ...
    } else if (error.message.includes('Hardware sudah terikat')) {
      errorResponse.error = 'HARDWARE_ALREADY_USED';
...
// Multer config untuk upload logo toko
const logoStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
    const extension = path.extname(file.originalname);
    cb(null, 'store-logo-' + uniqueSuffix + extension);
  }
});
...
}).single('logo'); // Single file upload dengan field name 'logo'
...
}).single('image'); // Nama field dari form di frontend

// POST to upload promo video
app.post('/api/settings/upload-logo', authenticateToken, (req, res) => {
  uploadLogo(req, res, async (err) => {
    ...
    try {
      ...
      // Simpan path ke database di tabel settings
      await pool.query(
        'INSERT INTO settings (`key`, `value`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `value` = ?',
        ['store_logo_path', logoPath, logoPath]
      );
      ...
      res.status(500).json({ message: `Gagal menyimpan logo ke database: ${dbError.message}` });
    }
  });
});
...
// GET store logo path
app.get('/api/settings/store-logo', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT value FROM settings WHERE `key` = ?', ['store_logo_path']);
    if (rows.length > 0) {
      res.json({ logoPath: rows[0].value });
    } else {
      res.json({ logoPath: null }); // Default ke null jika belum ada logo
    }
  } catch (e) {
    console.error('Error fetching store logo path:', e);
    res.status(500).json({ message: 'Gagal mengambil path logo toko.' });
  }
});

// GET general settings (including store logo)
app.get('/api/settings/general', async (req, res) => {
  try {
    ...
    const settings = {};
    rows.forEach(row => {
      // Convert database key to camelCase for frontend
      const camelKey = row.key.replace(/_([a-z])/g, (match, letter) => letter.toUpperCase());
      settings[camelKey] = row.value;
    });
    
    // Map storeLogo to the correct key
    if (settings.storeLogoPath) {
      settings.storeLogo = settings.storeLogoPath;
      delete settings.storeLogoPath;
    }
    
    res.json(settings);
  } catch (e) {
    console.error('Error fetching general settings:', e);
    res.status(500).json({ message: 'Gagal mengambil pengaturan umum.' });
  }
});

// POST save general settings (including store logo from form)
app.post('/api/settings/general', authenticateToken, async (req, res) => {
  try {
    ...
    const settingsMap = {
      ...
      'running_text_bg_color': runningTextBgColor,
      'running_text_text_color': runningTextTextColor
    };
    ...
    res.json({ message: 'Pengaturan umum berhasil disimpan.' });
  } catch (e) {
    console.error('Error saving general settings:', e);
    res.status(500).json({ message: 'Gagal menyimpan pengaturan umum.' });
  }
});
...
app.delete('/api/purchase-returns/:id', authenticateToken, authorizeAdminOrManager, async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const [returnCheck] = await conn.query('SELECT id FROM purchase_returns WHERE id = ?', [id]);
    if (!returnCheck.length) {
      await conn.rollback();
      return res.status(404).json({ message: 'Retur pembelian tidak ditemukan' });
    }
    
    // Delete purchase return items first (foreign key constraint)
    await conn.query('DELETE FROM purchase_return_items WHERE return_id = ?', [id]);
    
    // Delete the main purchase return record
    const [result] = await conn.query('DELETE FROM purchase_returns WHERE id = ?', [id]);
    
    await conn.commit();
    res.json({ message: 'Retur pembelian berhasil dihapus' });
    
  } catch (e) {
    await conn.rollback();
    console.error('Delete purchase return error:', e);
    res.status(500).json({ message: 'Gagal menghapus retur pembelian: ' + e.message });
  } finally {
    conn.release();
  }
});
...
// GET QUICK PRODUCTS (New version, reads from quick_products table)
...
// GET Quick Products for settings page
app.get('/api/settings/quick-products', authenticateToken, authorizeAdminOrManager, async (req, res) => {
  try {
    const query = `
      SELECT 
        p.id,
    ...
    console.error('Error fetching quick products for settings:', e);
    res.status(500).json({ message: 'Gagal mengambil data produk cepat.' });
  }
});

// POST to update Quick Products
app.post('/api/settings/quick-products', authenticateToken, authorizeAdminOrManager, async (req, res) => {
  const { quickProducts } = req.body; // Expect an array of { product_id, display_order, color }
  ...
  try {
    ...
    // 1. Clear the existing quick products
    await conn.query('DELETE FROM quick_products');

    // 2. Insert the new list
    if (quickProducts.length > 0) {
      const values = quickProducts.map(p => [p.product_id, p.display_order, p.color]);
      await conn.query(
        'INSERT INTO quick_products (product_id, display_order, color) VALUES ?',
        [values]
      );
    }

    await conn.commit();
    res.json({ message: 'Pengaturan produk cepat berhasil disimpan.' });

  } catch (e) {
    await conn.rollback();
    console.error('Error saving quick products:', e);
    res.status(500).json({ message: `Gagal menyimpan pengaturan: ${e.message}` });
  } finally {
    conn.release();
  }
});
...
// POST
app.post('/api/products', authenticateToken, authorizeAdminOrManager, (req, res) => {
  uploadProductImage(req, res, async (err) => {
    ...
    try {
      ...
      const [result] = await conn.query(
        'INSERT INTO products (name, price, type, jenis, ukuran, keyword, stock, harga_beli, barcode, image_url) VALUES (?,?,?,?,?,?,?,?,?,?)',
        [name, price, type, jenis, ukuran, keyword, stock || 0, harga_beli || 0, barcode || null, imageUrl]
      );
      const newId = result.insertId;

      let finalBarcode = barcode;
      if (!barcode) {
        finalBarcode = `DMBRG${String(newId).padStart(6, '0')}`;
        await conn.query('UPDATE products SET barcode = ? WHERE id = ?', [finalBarcode, newId]);
      }

      await conn.commit();
      res.status(201).json({ id: newId, barcode: finalBarcode, image_url: imageUrl, ...req.body });
    } catch (e) {
      await conn.rollback();
       if (req.file) {
        try {
...
// PUT
app.put('/api/products/:id', authenticateToken, authorizeAdminOrManager, (req, res) => {
  uploadProductImage(req, res, async (err) => {
    if (err) {
      return res.status(400).json({ message: err.message });
    }
    ...
    try {
      ...
      if (req.file) {
        const [rows] = await conn.query('SELECT image_url FROM products WHERE id = ?', [id]);
        if (rows.length > 0 && rows[0].image_url) {
          const oldImagePath = path.join(publicPath, rows[0].image_url);
          try {
      ...
      let sql = 'UPDATE products SET name=?, price=?, type=?, jenis=?, ukuran=?, keyword=?, stock=?, harga_beli=?, barcode=?';
      const params = [name, price, type, jenis, ukuran, keyword, stock, harga_beli, barcode];

      let newImageUrl = null;
      if (req.file) {
        newImageUrl = `/products/${req.file.filename}`;
        sql += ', image_url=?';
      ...
      const responsePayload = { message: 'Produk diperbarui' };
      if (newImageUrl) {
        responsePayload.image_url = newImageUrl;
      }
      ...
      if (req.file) {
        try {
          ...
          console.error("Error deleting new image after DB error:", unlinkErr);
        }
      }
      if (e.code === 'ER_DUP_ENTRY' && e.message.includes('barcode')) {
        return res.status(409).json({ message: 'Barcode sudah digunakan.' });
